...
def read_notes(notes, filter_date=None):
    if filter_date:
        filtered_notes = [note for note in notes if note.create_date.date() == filter_date]
    else:
...
while True:
    ...
    print("6. Просмотреть заметку по ID")
    print("7. Выход")
    ...
    if choice == '1':
        ...
        date_str = input("Введите дату для выборки заметок (в формате ГГГГ-ММ-ДД): ")
